...
module Datapath (
    ...
    input  logic [ 2:0] RFWiteDataScrMuxSel,    input  logic [ 2:0] extType,
    input  logic [ 3:0] immRe,
    input  logic        branch,
    ...
    immRE U_immRe_RAMrdata (
        .immRe(immRe),
        .RAM_i_data(dataMemRData),
        .RAM_o_data(w_dataMemRData)
    );
    immRE U_immRe_RAMwdata (
        .immRe(immRe),
        .RAM_i_data(w_RegFileRData2),
        .RAM_rdata(dataMemRData),
        .RAM_o_data(dataMemWData)
    );
...
module immRE (
    input  logic [ 3:0] immRe,
    input  logic [31:0] RAM_i_data,
    input  logic [31:0] RAM_rdata,
    output logic [31:0] RAM_o_data
);
    ...
    always_comb begin : blockName
        ...
        case (immRe)
            //IL-Type
            4'b0000: RAM_o_data = {{24{RAM_i_data[7]}}, RAM_i_data[7:0]};  //LB
            4'b0001:RAM_o_data = {{16{RAM_i_data[15]}}, RAM_i_data[15:0]};  //LH
            4'b0010: RAM_o_data = RAM_i_data;  // LW
            4'b0100: RAM_o_data = {24'd0, RAM_i_data[7:0]};  //LBU
            4'b0101: RAM_o_data = {16'd0, RAM_i_data[15:0]};  //LHU
            //S-type
            4'b1000: RAM_o_data = {RAM_rdata[31:8], RAM_i_data[7:0]};  //SB
            4'b1001: RAM_o_data = {RAM_rdata[31:16],RAM_i_data[15:0]};  //SH
            4'b1010: RAM_o_data = RAM_i_data;  //SW
            default: RAM_o_data = 32'dx;
        endcase
    end
endmodule
